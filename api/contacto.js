...
export default async function handler(req,res){
  ...
  try{
    ...
    const subject=`Nueva consulta MedionTech - 123pcsolutions.com.es | ${modelo||'Ordenador Medion'}`;
    const html=`<div style="font-family:Arial,sans-serif;line-height:1.6;color:#0a2a4a"><h2 style="color:#0072ce">Nueva consulta desde MedionTech</h2><p><strong>Nombre:</strong> ${safe.nombre}</p><p><strong>Teléfono:</strong> ${safe.telefono}</p><p><strong>Email:</strong> ${safe.email}</p><p><strong>Modelo Medion:</strong> ${safe.modelo}</p><p><strong>Descripción de la avería:</strong><br>${safe.mensaje}</p><hr><p style="font-size:12px;color:#5c6b7a">Enviado desde el formulario de 123pcsolutions.com.es</p></div>`;
    const mime=[`From: MedionTech <${from}>`,`To: ${to}`,`Reply-To: ${email}`,`Subject: ${encodeHeader(subject)}`,'MIME-Version: 1.0','Content-Type: text/html; charset="UTF-8"','Content-Transfer-Encoding: 8bit','',html].join('\r\n');
